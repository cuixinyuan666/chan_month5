...
2. **早期返回检查**：
   - `len(children) < 3` → 更新方向状态后返回
   - 无有效 `active_dir` → 返回
   - `len(confirmed_children) < 3` → 更新方向状态后返回
   - `len(seq) < 3` → 更新方向状态后返回
   - 无偶数索引（2, 4, 6...）→ 更新方向状态后返回
3. 通过 `build_alternating_child_sequence()` 构建交替子序列（仅已确认且 `idx >= start_bi_idx` 且 `end_x <= display_x`）
4. 对每个偶数索引的 D 点，用之前的 A-B-C 回调比计算节奏价
5. 输出 `step_rhythm_lines` 数组，每个元素包含 `value`、`ratio`、`round_current`、`round_ref`、`layer`、`key`、`label`、`dir` 等
...
**标签**：`节奏线:3 上 12.345`（行数 + 方向 + 第一条线最新值）
...
- [ ] 节奏线 key 的去重逻辑是否正确（`step_rhythm|label|round_ref`）
