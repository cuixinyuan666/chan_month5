...
| `chan-buy-sell-point` | `.trae/skills/chan-buy-sell-point/SKILL.md` | `BuySellPoint/BS_Point.py`（CBS_Point 数据结构）、`BuySellPoint/BSPointList.py`（CBSPointList.cal 主流程 / 一类/二类/三类/类二判定）、`BuySellPoint/BSPointConfig.py`（CBSPointConfig / CPointConfig 14 项配置）、`Common/CEnum.py`（BSP_TYPE / MACD_ALGO 枚举）、`KLine/KLine_List.py`（cal_seg_and_zs 末尾 bs_point_lst.cal）、`a_replay_trainer.py`（build_level_bsp / serialize_bsp_collection / make_bsp_item / bsp_history / _bsp_key / sync_bsp_history / _judge_bsp_against_all / after_step_update / chart_lazy_bsp_enabled / bsp_levels）、`a_rust_core/chan-core/src/state.rs`（Rust 侧 BspPoint/BspList/cal_bi_level/cal_seg_level，仅 T1/T1P/T2）、`a_rust_core/chan-core/src/enums.rs`（BspType）、`a_rust_core/src/lib.rs`（bsp_items_from_list / bsp_delta_reset / bsp_delta_collect PyO3 绑定）、前端 drawBsp / drawBottomSignals / buildBottomSignalGroups / bottomBspRowKey 渲染、payload 字段 bsp |
