...
def _default_chan_month5_vault():
    """<obsidian.json vault>/chan-month5，否则回退 Documents/Obsidian Vault。"""
    appdata = os.environ.get("APPDATA")
    if appdata:
        cfg = os.path.join(appdata, "obsidian", "obsidian.json")
        if os.path.isfile(cfg):
            try:
                with open(cfg, "r", encoding="utf-8") as f:
                    data = json.load(f)
                vaults = data.get("vaults") or {}
                open_id = data.get("open") or data.get("lastOpen")
                root = None
                if open_id and open_id in vaults:
                    root = vaults[open_id].get("path")
                if not root:
                    for v in vaults.values():
                        if v.get("open"):
                            ...
                            break
                if root:
                    return os.path.normpath(os.path.join(root, "chan-month5"))
...
def main():
    ...
    default_vault = _default_chan_month5_vault()
