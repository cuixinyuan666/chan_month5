...
### 12.1 数据形态模式 (data_form_mode)

| 模式 | 说明 |
|------|------|
| `traditional` | 传统模式，按时间顺序 |
| `quantity` | 按数量Q聚合K线，前面等分，余数归入最后一组 |
| `tick_traditional` | 分笔价格合成传统模式，从分笔数据合成K线后按时间顺序 |
| `tick_quantity` | 分笔价格合成数量模式，从分笔数据合成K线后按数量聚合 |

### 12.2 数据喂入模式 (data_feed_mode)

控制K线如何喂给缠论引擎：

| 模式 | 说明 |
|------|------|
| `step`（默认） | 逐步喂入：通过 `step_load()` 逐根喂K线，支持步进/回退/模拟操盘 |
| `unified` | 统一喂入：通过 `trigger_load()` 一次性喂入全部K线，仅用于看图，**不支持模拟操盘**（买入/卖出/做空/平空均被禁用） |

**unified 模式的限制**：
- 前端买入/卖出/做空/平空按钮灰色不可用
- API `/api/buy`、`/api/sell`、`/api/short`、`/api/cover` 返回 400 错误
- 适用于仅查看缠论结构、不需要交互式回放的场景
