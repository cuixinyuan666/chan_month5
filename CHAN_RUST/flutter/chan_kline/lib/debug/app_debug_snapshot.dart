...
class AppDebugSnapshot {
  static String build({
    ...
    buf.writeln(
      'N段流水线：笔=1段，线段=2段…；逐K当下冻结，未来结构不回写旧标签。',
    );
    buf.writeln(
  ...
  static void _writeLevels(StringBuffer buf, List<LevelBundle> levels) {
    ...
    for (final lv in levels) {
      ...
      buf.writeln(
        'L${lv.level}段：policy=${lv.segmentPolicy}；'
  ...
  static void _writeDllDiag(
    ...
    if (!hasLevels && barFeatures.isNotEmpty) {
      buf.writeln('⚠ levels 为空但已有 bar_features，可能旧 DLL 或计算未产出 N 段层');
    }
    buf.writeln();
  }
    ...
    if (feat == null) {
      ...
      if (feat.levels.isNotEmpty) {
        ...
        for (final snap in feat.levels) {
          buf.writeln(
            '    L${snap.level} unit=${snap.unitIdx} dir=${snap.unitDir} '
