...
/// 从已归一化信号跑：订单 → 下一根 K0 开盘成交 → 单仓多头 → 交易记录。
/// displayKn 只留在信号上，成交钟永远是 K0。
MiniLoopResult runMiniLoopFromSignals({
  ...
  for (final sig in tradable) {
    ...
    if (sig.side == TradeSide.buy) {
      ...
      final price = slippage.apply(next.open);
...
MiniLoopResult runMiniLongOnlyLoop({
  required List<KlineBar> bars,
