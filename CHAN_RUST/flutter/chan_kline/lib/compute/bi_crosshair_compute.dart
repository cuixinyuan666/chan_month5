...
/// 十字线 tooltip 用笔 K 字段（当前步前缀归属，非 ML 逐步冻结）。
class BiCrosshairTooltipFields {
  final int biIdx;
  ...
  const BiCrosshairTooltipFields({
    required this.biIdx,
    required this.biMergeInnerSeq,
    required this.biMergeCount,
    required this.biOpen,
    required this.biHigh,
    required this.biLow,
    required this.biClose,
    required this.biVolume,
...
int _nextBiAfterReplay(List<KlineBar> bars, List<BiSegment> biSegments) {
  var nextBi = 0;
...
/// 当前步前缀下：分钟 K 所属笔 K（进行中笔优先，否则已确认笔区间）。
BiVirtualBar? owningBiAtCurrentStep(
  List<KlineBar> bars,
  List<BiSegment> biSegments,
  List<BiConfirmSignal> biConfirms,
  int minuteIdx,
  ...
  final currentEnd = bars.length - 1;
  final nextBi = _nextBiAfterReplay(bars, biSegments);
  ...
  if (prov != null && prov.x1 <= minuteIdx && minuteIdx <= prov.x2) {
    return prov;
  }
  for (var i = biSegments.length - 1; i >= 0; i--) {
    final vb = computeBiVirtualBars(bars, [biSegments[i]]).firstOrNull;
    if (vb != null && vb.x1 <= minuteIdx && minuteIdx <= vb.x2) {
      return vb;
    }
  }
  return null;
...
_HlCombineStepState _mergeSnapAtCurrentStep(
  List<KlineBar> bars,
  List<BiSegment> biSegments,
  List<BiConfirmSignal> biConfirms,
) {
  ...
  if (bars.isNotEmpty) {
    ...
    if (prov != null) {
      snap.updateProvisional(_unitFromVb(prov), prov.idx);
    }
  }
  return snap;
}

/// 十字线 tooltip：当前步各分钟 K → 所属笔 K 展示字段（ML 仍用 barFeatures 冻结 bi_*）。
Map<int, BiCrosshairTooltipFields> buildBiCrosshairTooltipMap(
  List<KlineBar> bars,
  List<BiSegment> biSegments,
  List<BiConfirmSignal> biConfirms,
) {
  if (bars.isEmpty) return const {};
  final snap = _mergeSnapAtCurrentStep(bars, biSegments, biConfirms);
  final out = <int, BiCrosshairTooltipFields>{};
  for (final b in bars) {
    final owning = owningBiAtCurrentStep(
      bars,
      biSegments,
      biConfirms,
      b.idx,
    ...
    final cross = snap.crosshairSnapshot(owning.idx);
    out[b.idx] = BiCrosshairTooltipFields(
      biIdx: owning.idx,
      biMergeInnerSeq: cross.biMergeInnerSeq,
      biMergeCount: cross.biMergeCount,
      biOpen: owning.open,
      biHigh: owning.high,
      biLow: owning.low,
      biClose: owning.close,
      biVolume: _biVolume(bars, owning.x1, owning.x2),
      biCombineHigh: cross.biCombineHigh,
      biCombineLow: cross.biCombineLow,
      biCombineFx: cross.biCombineFx,
    );
  }
  return out;
}
