...
- **Kn一类BS 同枢极值（全层同构+镜像）**：同一中枢框内仅建框/严格新低（买）或新高（卖）标一类；等高/更弱归二类。B 比本枢已见最低 low、S 比已见最高 high；跳过更高低/更低高时**不**改参照。
- **Kn二类BS（方案A·全层同构）**：与一类同一资格中枢框；买 `low≥已见最低`→`2Ba…`，卖镜像→`2Sa…`；二类字母随一类建框/新极值同步复位。会话冻结/双键/副图「Kn二类BS」与一类同构。
